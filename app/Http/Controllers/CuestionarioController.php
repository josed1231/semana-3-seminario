...
class CuestionarioController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // 2. Guardar o actualizar datos base del Estudiante (username es la cédula / código de estudiante)
            $codigoEstudiante = $user->username ?? $user->codigo_estudiante ?? (string)$user->id;
            ...
            $respuestas = [
                'genero'                    => $request->input('genero'),
                'victima_conflicto'         => $request->input('victima_conflicto'),
            ...
            // Al guardar en 'riesgo()', el Observer reaccionará automáticamente
            ...
            // 5. Generación directa de la Orientación Psicológica (Protegido)
            try {
                ...
                Log::error("Error generando orientación en cuestionario: " . $e->getMessage());
            }

            // 6. Disparar evento de notificación (Protegido si el servicio de correo falla)
            try {
                event(new EstudianteActualizado($estudiante, 'cuestionario'));
            } catch (\Exception $e) {
                Log::error("Error disparando evento EstudianteActualizado: " . $e->getMessage());
            }

            return redirect()->route('cuestionario.success');

        } catch (\Exception $e) {
            Log::error("Error general guardando cuestionario: " . $e->getMessage());
            return back()->withErrors(['error' => 'Ocurrió un inconveniente al registrar el cuestionario. Revisa los datos o vuelve a intentarlo.']);
        }
    }
    ...
    public function update(Request $request, $codigo_estudiante)
    {
        ...
        if ($academicoRaw || $socioRaw || $psicoRaw) {
            try {
                ...
                Log::error("Error actualizando orientación en update: " . $e->getMessage());
            }
        }
